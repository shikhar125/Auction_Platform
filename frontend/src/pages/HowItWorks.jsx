  ...
  return (
    <>
      <section className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col min-h-screen py-4 justify-center">
        ...
        >
          Discover How BidStorm Operates
        </h1>
        <div className="flex flex-col gap-4 my-5">
          {steps.map((element, index) => {
            return (
              <div
                key={index}
                className="bg-card rounded-md p-2 lg:p-5 flex flex-col gap-2 group hover:bg-secondary transition-all duration-300"
              >
                <div className="bg-card-foreground text-card p-3 text-xl rounded-full w-fit group-hover:bg-primary transition-all duration-300">
                  {element.icon}
                </div>
                <h3
                ...
                <p className="text-xl text-muted-foreground group-hover:text-secondary-foreground transition-all duration-300">
                  {element.description}
                </p>
              </div>
            );
